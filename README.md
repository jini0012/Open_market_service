...
- 상품 값을 000,000 이런식으로 작성해야하는데, 문자열내에 ','를 넣으려고 했으나 기존에 알던 메소드로는 구현하는데 어려움이 있어서 새로 배우게된 방법을 적용했다. `toLocaleString()` 을 사용하는 방법이 있지만 `Intl.NumberFormat` 로 언어에 맞는 숫자 서식을 지원하는 생성자를 사용해서 적용했다.
